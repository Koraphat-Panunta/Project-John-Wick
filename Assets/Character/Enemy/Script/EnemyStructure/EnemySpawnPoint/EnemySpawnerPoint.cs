...
public class EnemySpawnerPoint : MonoBehaviour
{
    ...
    public virtual Enemy SpawnEnemy(EnemyPoolManager poolManager, WeaponPoolManager weaponPoolManager, EnemySpawnerData data, EnemyDirector fallbackDirector)
    {
        ...
        if(enemy.TryGetComponent<EnemyDecision>(out EnemyDecision enemyDecision))
        {
            
            enemyDecision.enemyDecisionContext.SetRoleCommand(data.spawnRole);
            enemyDecision.enemyDecisionContext.SetCombatPhase(data.spawnCombatPhase);
        }

        switch (data.spawnCombatPhase)
        {
            case CombatPhase.Aware:
            case CombatPhase.Alert:
                {
                    enemy.SetTargetKnowPos(fallbackDirector.globalTargetKnowPos);
                    break;
                }
        }

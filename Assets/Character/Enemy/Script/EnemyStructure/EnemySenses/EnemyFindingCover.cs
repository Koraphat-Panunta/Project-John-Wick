...
public class EnemyFindingCover 
{
    ...
    //public List<CoverPoint> coverPositionEnemies = new List<CoverPoint>();
    //public CoverPoint coverPositionEnemy;
    ...
    //}
    //public bool FindingCover(Enemy _enemy )
    //{
    ...
    //            continue;
    //        }
    //        Vector3 CastDir = (new Vector3(col[i].bounds.center.x, _enemy.rayCastPos.position.y, col[i].bounds.center.z)-_enemy.rayCastPos.position).normalized;
    //        if (Physics.Raycast(_enemy.rayCastPos.position, CastDir,out RaycastHit hitInfo, Vector3.Distance(col[i].bounds.center, _enemy.rayCastPos.position), LayerMask.GetMask("Default")))
    //        {
    //            if (hitInfo.collider == col[i] &&Vector3.Dot(Vector3.up, hitInfo.normal) < 0.9f)
    //            {
    //                //Debug.DrawLine(_enemy.rayCastPos.position, hitInfo.point,Color.green);
    ...
    //    if (enemyCoverObstacles.Count > 0)
    //    {
    //        Vector3 targetPos = _enemy.targetKnewPos;
    ...
    //                    if (Physics.Raycast(AimPos, (targetPos - AimPos).normalized,out RaycastHit hitInfo, Vector3.Distance(AimPos, targetPos), LayerMask.GetMask("Default")+_enemy.targetMask))
    //                    {
    //                        if (hitInfo.collider.gameObject.layer == _enemy.targetMask)
    //                        {
    //                            coverPositionEnemies.Add(new CoverPosition(enemyCoverObstacle.coverPivotPos[i], CoverPos, AimPos));
    ...
    //            {
    //                Enemy enemyInCol = n.GetComponent<BodyPart>()._enemy;
    ...
    //    else
    //    {
    //        foreach (CoverPoint coverPositionEnemy in coverPositionEnemies)
    //        {

...
public class PainStateProceduralBodyConstraintNodeLeaf : BodyRotationConstraintNodeLeaf
{
    ...
    {
        this.painLookAtPos = root.position + root.forward * 2;
        this.timer = 0;
        base.Enter();
    }
    ...
    protected override void UpdateLookAtTarget()
    {
        ...
        this.bodyConstraint.SetLookPos(painLookAtPos);
    }
    ...
    {
        bodyConstraint.SetWeight(Mathf.Clamp(
            bodyConstraint.GetWeight() + (base.getOffsetChangedRate * Time.deltaTime), 0, this.intensity));
    }
}

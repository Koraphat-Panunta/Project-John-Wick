...
public class InWorldUIManager : MonoBehaviour,IInitializedAble
{
    ...
    public void InitailizedNode()
    {
        ...
        enemyStatusInWorldUIManageNodeLeaf = new EnemyStaggerStatusInWorldUIManageNodeLeaf(
            ...
            && this.player.executeGauge._gauge >= this.player.executeGauge.maxGauge
            && (player._currentWeapon.TryGetBulletCapacity(out BulletCapacity bulletCapacity)? bulletCapacity.curCount : 0) + (player._currentWeapon.chamber.isReadyShoot?1:0) > 0
            , mainCamera
            ,player
            ,executeInWorldUI);

...
public class TakeCoverTactic : IEnemyTactic
{
    ...
    public TakeCoverTactic(Enemy enemy)
    {
        ...
        //Debug.Log(_enemy + " EnterTakeCover");
    ...
    public void Manufacturing()
    {
        ...
        else if(coverPositionEnemy != null)
        {
            ...
            else if (isInCover == true)
            {
                ...
                //{
                //    _enemy.currentTactic = new HoldingTacticDecision(_enemy);
                //}
            }
        }
    ...
    private void PeekAndShoot(Vector3 peekPos,NavMeshAgent agent) 
    {
        ...
        //_enemy.weaponCommand.AimDownSight();
        //_enemy.enemyStateManager.ChangeState(_enemy.enemyStateManager._moveWithAgent);
        //enemyFiringPattern.Performing();

        enemy.enemyController.Rotate((enemy.targetKnewPos - enemy.transform.position).normalized, 6);
        //new RotateObjectToward().RotateTowardsObjectPos(_enemy.targetKnewPos, _enemy.gameObject, 6);
    ...
    private bool MoveToCover(Vector3 CoverPos, NavMeshAgent agent)
    {
        ...
        {
            //_enemy.enemyStateManager.ChangeState(_enemy.enemyStateManager._sprint);
    ...
    //{
    //    Collider[] other = Physics.OverlapSphere(_enemy.transform.position, 0.5f);
    //    foreach(Collider collider in other)
    //    {
    //        Enemy enemyNearBy;
    //        if(collider.TryGetComponent<ChestBodyPart>(out ChestBodyPart e))
    //        {
    //            enemyNearBy = e._enemy;
    //            if (enemyNearBy != _enemy)
    //            {
    //                return true;
    //            }
    //        }
